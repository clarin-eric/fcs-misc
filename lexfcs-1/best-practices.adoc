...
== Serialisation with the Lexical Data View
...
<2> Any `<lex:Value>` element without its own language attributes inherits language information from its parent  `<lex:Entry>`, here `@xml:lang="_de_"`.
<3> `@xml:lang="_de_"` is redundant.
...
=== Contextualise Value contents for improved interpretability

// senseRef: dornseiff, PWN with www-link; description of acronyms (PWN, …)

The example <<example-senseref-contextualisation>> shows how plain text content can be semantically enriched by explicitly stating the used vocabulary or by referencing the value's external definition via attributes `@vocabRef` or `@vocabValueRef`.

.Contextualisation of senseRef Values
[%linenums#example-senseref-contextualisation,xml]
----
